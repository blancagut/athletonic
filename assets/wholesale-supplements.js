(function () {
  ...
  function renderProducts(products, append) {
    ...
    const html = products
      .map((product) => {
        ...
        const colorOptions = optionSelect(product.colors, "Flavor", "data-card-color");
        return `
          <article class="wholesale-line" data-product-id="${escapeHtml(product.id)}">
            ...
            <div class="wholesale-line__options">
              ...
              ${optionChips(product.colors, "Flavors")}
  ...
  function selectedOptionsForLine(line, product) {
    ...
    if (color && color.value) selected.Flavor = color.value;
    else if (Array.isArray(product.colors) && product.colors.length === 1) selected.Flavor = product.colors[0];
    return selected;
  }
